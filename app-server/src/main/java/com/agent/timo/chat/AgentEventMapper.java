...
public final class AgentEventMapper {
    ...
    private List<ChatDelta> result(Msg msg) {
        ...
        if (reason == GenerateReason.TOOL_SUSPENDED) {
            // bash 危险命令已迁框架 Permission 体系（M1-8：checkPermissions ASK →
            // RequireUserConfirmEvent → ui_request → /api/hitl 回路）。本分支只剩真正的
            // 外部执行工具（externalTool，M2+）：它们只产生 suspended 结果块、没有应答通道，
            // 显式报错而非静默收尾。
            deltas.add(ChatDelta.Error.of("工具已挂起等待外部执行，当前版本尚未接通该应答通道（M2）"));
        }
        return deltas;
    }
